...
int16 Hex::WriteFlash(int16 axis, vector<INTEL_HEX_FRAME>* frame, void(*tpfUpdataProgressPt)(void*, int16*), void* ptrv, int16 com_type, int16 stationId)
{
	int16 progress;
	int16 index = 0;
	...
	{
		if (GTSD_CMD_FlashWrite(axis, &(*it), com_type,stationId) != 0)
		{
			return -1;
		}
...
int16 Hex::ReadFlash(int16 axis, vector<INTEL_HEX_FRAME>* frame, void(*tpfUpdataProgressPt)(void*, int16*), void* ptrv, int16 com_type, int16 stationId)
{
	int16 progress;
	int16 index = 0;
	int32 vector_size = frame->size();
	...
	{
		INTEL_HEX_FRAME tmp;
		if (GTSD_CMD_FlashRead(axis, &(*it), &tmp, com_type,stationId) != 0)
		{
			return -1;
		}
