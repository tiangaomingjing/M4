...
int16 CPlotWave::InitDspWaveVar(int16 dsp_number, WAVE_BUF_PRM& wave)
{
	...
	dataFrame_Lenth_dsp[dsp_number] = (dataFrame_Lenth_dsp[dsp_number] >> 1);					//转化到int16的个数
...
int16 CPlotWave::ParseDspPlotWaveData(int16 dsp_number, WAVE_BUF_PRM& wave, int32 number)
{
	...
	if (raw_Data_GoBack_Flag_dsp[dsp_number] == true)
	{
		...
		for (j = 0; j < (number + raw_Data_ParseLeftNumber_dsp[dsp_number]);)
		{
			...
			if (syncFrame_Flag_dsp[dsp_number] == 1)//假如出现一个同步帧
			{
	...
	else
	{
		...
		for (i = raw_Data_ParseIndex_dsp[dsp_number]; i < (raw_Data_Index_dsp[dsp_number]);)
		{
			...
			{
				raw_Data_ParseIndex_dsp[dsp_number] = i;								//记录该位置的值用于下次的i的起始值			
				waveLine_Index_dsp[dsp_number] = 0;								//曲线的index复位				
				raw_Data_ParseLeftNumber_dsp[dsp_number] = raw_Data_Index_dsp[dsp_number] - i;		//更新剩下的个数
				break;																//跳出该循环
			}
			//首先判断数据中是否有同步帧
			//假如找到同步帧的低位，连续又找到同步帧的高位
			...
			{
				syncFrame_Flag_dsp[dsp_number] = 1;								//说明是同步帧，标志置位
				i += 2;																//更新i
				...
				recv_SyncFrame_Num_dsp[dsp_number]++;											//记录收到的同步帧个数
			...
			{
				waveLine_Index_dsp[dsp_number] = 0;
				recv_DataFrame_Num_dsp[dsp_number]++;												//完整帧的个数加1
			}
			...
			if (syncFrame_Flag_dsp[dsp_number] == 1)//假如出现一个同步帧		
			{
				if (waveLine_Index_dsp[dsp_number] != 0)
				{
					...
					{
						(waveLine_WrIndex_dsp[dsp_number][p])++;
						waveLine_WrNumOnce_dsp[dsp_number][p]++;								//该组数据实际写入曲线中的个数
						if ((waveLine_WrIndex_dsp[dsp_number][p]) >= WAVE_LINE_BUFFER_LENTH)
						{
							(waveLine_WrIndex_dsp[dsp_number][p]) = 0;					//写指针复位
						}
					}
					waveLine_Index_dsp[dsp_number] = 0;											//重置曲线条数index					
				}
			}
			//将当前值作为数据写入,根据占据的字节数不同，从buffer中取得个数不同
			switch (wave.inf[waveLine_Index_dsp[dsp_number]].bytes)
...
//////////////////////////////////////////////////////////////////////////
	...
	if (read_num != 0)
	{
		...
		if (tmp >= (RAW_BUFFER_LENTH))
		{
			...
			raw_Data_GoBack_Flag_dsp[dsp_number] = true;		//过圈标志
		}
		else
		{
			...
			raw_Data_GoBack_Flag_dsp[dsp_number] = false;		//过圈标志
