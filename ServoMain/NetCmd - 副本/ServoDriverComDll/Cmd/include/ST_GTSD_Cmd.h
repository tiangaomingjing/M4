...
const int32						COM_AXIS_MAX			= 240;					//轴最大个数为4	
...
//eeprom 基地址定义
//pcdebug 地址和 等环网挂的地址不同
const int32						FPGA_EEPROM_PCDEBUG_BASEADDR	= 0x0800; //short地址 byte地址0x1000-0x10ff

//等环网地址
const int32						FPGA_EEPROM_RNNET_BASEADDR		= 0x1000; //byte地址 

//byte地址，对于等环网和pcdebu都是一样的偏移，用于pcdebug时需要发下去short地址，所以要右移一位
const int32						FPGA_EEPROM_CTL					= 0x00;   //控制寄存器
//D0:  eeprom 读写控制位
//	   1'b0 :写eeprom操作；
//	   1'b1 :读eeprom操作；
//D1 - D3: eeprom芯片硬件地址，6轴主板为3'b000;
//D4: eeprom读写模块使能位
//	  1'b1：使能eeprom读写模块；
//	  1'b0: 不使能；
//D5: eeprom芯片软复位位
//	  1'b1：同时D4位为1，eeprom芯片软复位
//	  1'b0: 不复位；
//D6 - D15：保留；
const int32						FPGA_EEPROM_WRITE_ADDR			= 0x02;  //写地址寄存器
//D0 - D7：写eeprom的地址；
//D8 - D15：保留；
const int32						FPGA_EEPROM_WRITE_DAT			= 0x04;  //写数据寄存器
//D0 - D7：写eeprom固定地址的数据；
//D8 - D15：保留；
const int32						FPGA_EEPROM_READ_ADDR			= 0x06;  //读地址寄存器
//D0 - D7：读eeprom的地址；
//D8 - D15：保留；
const int32						FPGA_EEPROM_READ_DAT			= 0x0a;  //读数据寄存器
//D0 - D7：读eeprom固定地址的数据；
//D8 - D15：保留；
const int32						FPGA_EEPROM_STATUS				= 0x08;  //状态寄存器
//D0: 读写错误标志位，即读写操作无应答。1'b0:无错误，1'b1 : 读写无应答；
//D1 : 读写完成标志 1'b0:没完成，1'b1 : 读写完成
