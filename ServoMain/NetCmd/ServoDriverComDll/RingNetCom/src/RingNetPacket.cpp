	...
	case PACKET_USER:
		...
		case 0:
			...
			switch (dspId)
			{
				//DSPA
			case RN_DSP_CH_ID:
				g_plotWave->m_rn_noTimer->GetPlotWaveDataFromRn(GTSD_DSP_A + pRx->src_id - 1);
				break;
				//DSPB
			case RN_PCI_CH_ID:
				g_plotWave->m_rn_noTimer->GetPlotWaveDataFromRn(GTSD_DSP_B + pRx->src_id - 1);
				break;
			default:
				break;
			}
		...
		case 1:
			...
			//和FPGA通信已经连接成功
			if (g_AbsCom->com_flag == 1)
			{
				//假如是广播并且通道是FPGA,记录拓扑结构
				if ((g_cur_cmd_log.des_id == 0xff) && (pRx->header.bit.src_ch == RN_FPGA_CH_ID))
				{
					//获取站类型和站号
					STA_LOG tmp;
					tmp.sta_msg = (Uint16)(parsePacketCmd[7]);
					tmp.sta_id = (Uint16)(parsePacketCmd[11]);
					sta_log.push_back(tmp);
				}
			}
