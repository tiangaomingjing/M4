...
{
	p_Net = NULL;
	Com_Rcv_Event = CreateEvent(NULL, FALSE, FALSE, NULL);			//Create an Auto-Reset Event,第三个参数是初始状态，true表示开始信号就是有信号的，这里设置为false，开始的wait就等不到信号了，必须setevent之后才有信号
}
		...
		if (0 == p_Net->tx_packet())																				//发送请求
		{
			if (needReq == RN_NEED_REQ)
			{
				...
				if (WaitForSingleObject(Com_Rcv_Event, delay_time) == WAIT_OBJECT_0)
				{
					...
					memcpy_s(pData, num*sizeof(int16), &parsePacketCmd[3], num*sizeof(int16));
				}
				else
				{
					ret = RN_Net_Rt_Timeout;
