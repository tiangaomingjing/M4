...
//返回2：速度不是千兆
//返回3：没有找到对应的网卡
//该函数在open以后调用，如果open失败就不需要调用该函数了，调用open成功后判断是否是千兆连接
...
SERVODRIVERCOMDLL_API short GTSD_CMD_XmlWriteFile(int16 axis, char* pFileNameList[], int pFileTypeList[], int file_num,
	void(*tpfUpdataProgressPt)(void*, short*), void* ptrv, short& progress, int16 com_type = GTSD_COM_TYPE_RNNET, int16 stationId = 0xf0);
