...
//            driveEncoder.activeNow.connect(onActiveNow);
    ...
    Rectangle{
        ...
        property AbstractEncoderItem yaskawaItem:YaskawaEncoderItem{
        }

        property var items: [duomoItem,nikangItem,haidehanItem,sanxieItem,songxiaItem,yaskawaItem]
...
//                    console.log("root.errorcode="+root.errorCode);
                    m_encoderWarnningBlock.visible=true;
//                    m_encoderWarnningMsg.text=m_encoderCfg.absEncoderItem.errorString(root.errorCode);//TEST
                    //编码器丢失就不用显示其它的信息了
                    m_encoderWarnningMsg.text=root.encLostFlag?qsTr("编码器丢失!"):m_encoderCfg.absEncoderItem.errorString(root.errorCode);
                }
